...
def kg_data_process():
    ...
    for i in range(len(head_train)):
        if i % 100000 == 0:
            print(i)
        if head_train[i] in train_set:
            if relation_train[i] not in relation_list:
                relation_list[relation_train[i]] = r_n
                r_n = r_n + 1
            if tail_train[i] not in tail_list:
                tail_list[tail_train[i]] = t_n
                t_n = t_n + 1

            f.write(str(head_train[i]) + ' ' + str(relation_list[relation_train[i]]) + ' ' + str(
                tail_list[tail_train[i]]) + '\n')
